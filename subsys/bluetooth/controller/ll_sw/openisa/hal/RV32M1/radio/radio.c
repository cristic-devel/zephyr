...
void *radio_ccm_rx_pkt_set(struct ccm *ccm, u8_t phy, void *pkt)
{
	...
	cauv3_ctx_ccm.rx_pkt_in = (struct pdu_data *)_pkt_scratch;
	cauv3_ctx_ccm.rx_pkt_out = (struct pdu_data *)pkt;
...
u32_t radio_ccm_is_done(void)
{
	...
	if (cauv3_ctx_ccm.rx_pkt_in->len > 4) {
		auth_mic = (u8_t *)cauv3_ctx_ccm.rx_pkt_in + 2 +
				cauv3_ctx_ccm.rx_pkt_in->len - 4;
		aad = *(u8_t *)cauv3_ctx_ccm.rx_pkt_in & RADIO_AESCCM_HDR_MASK;
		status = CAU3_AES_CCM_DecryptTag(CAU3, &handle,
				(u8_t *)cauv3_ctx_ccm.rx_pkt_in + 2,
				(u8_t *)cauv3_ctx_ccm.rx_pkt_out + 2,
				cauv3_ctx_ccm.rx_pkt_in->len - 4,
				(const u8_t *)cauv3_ctx_ccm.nonce, 13,
				&aad, 1, auth_mic, 4);
		if (status != kStatus_Success) {
		...
		cauv3_ctx_ccm.rx_pkt_out->len -= 4;

	} else if (cauv3_ctx_ccm.rx_pkt_in->len == 0) {
		/* Just copy input into output */
		*cauv3_ctx_ccm.rx_pkt_out = *cauv3_ctx_ccm.rx_pkt_in;
		cauv3_ctx_ccm.auth_mic_valid = 1;
		cauv3_ctx_ccm.empty_pdu_rxed = 1;
	} else {
		return 0;   /* length allowed only 0, not 1,2,3,4 */
	}
